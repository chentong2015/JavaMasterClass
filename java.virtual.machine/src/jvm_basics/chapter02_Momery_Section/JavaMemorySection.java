...
public class JavaMemorySection {
    ...
    //          1.2.1 局部变量表：表中第一个位置_0存储的是this(当前方法的对象)
    //                          objRef = new MyClass(); objRef存放对象在堆中的内存地址
    //                          istroe_1 一般是编译期可知的JVM基本数据类型
    //          1.2.2 操作数栈 : 将指定的数据值(操作数)压入到的内存空间位置, 数据运算过程中临时的内存空间
    //                          iconst_1
    //          1.2.3 动态链接 : C++底层源码概念，将符号引用转变成直接引用，然后存储
    //                          Java里将方法名视为符号，对符号进行解析，通过"符号对应的内存地址"在方法区中找到方法的具体代码
    //          1.2.4 方法出口 : 方法执行完要返回的位置，始终位置栈帧的栈底
    ...
    //
    // 3 方法区: 线程共享的内存区域: 存放常量，静态变量，类的信息(对象内存数据)，常量池
    //    TODO: 常量池
    ...
    //      >= jdk1.8 方法区的具体实现叫元空间，放在本地内存(OS内存，直接内存，native memory) ==> 操作系统回收，减少GC压力
    //    为什么要用元空间代替永久代 ?
    //      永久代，放在堆上，容易产生OOM
