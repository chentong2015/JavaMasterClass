...
public class JavaMemorySection {
    ...
    //          1.3.1 局部变量表：表中第一个位置_0存储的是this(当前方法的对象)      ==> 存放编译期可知的java虚拟机基本数据类型/局部变量槽(Slot)表示
    //                          objRef = new MyClass() 存放对象在堆中的内存地址
    //          1.3.2 操作数栈 : 将指定的数据值(操作数)压入到的内存空间位置, 数据运算过程中临时的内存空间
    //          1.3.3 动态链接 : 将方法名视为符号，对符号进行解析，通过"符号对应的内存地址"在方法区中找到方法(具体代码)
