...
public class JavaMemorySection {
    ...
    // 2. 方法区: 线程共享的内存区域
    //    3.1 class对象，方法对象集合(main方法对象，add方法对象...)
    //    3.2 TODO: 存放常量，静态变量，类的信息(对象内存数据)，常量池
    //    常量池:
    //      class文件常量池，运行时常量池，全局常量池
    //      jdk1.8之前字符串放在永久代，jdk1.8之后移动到堆中
    //    TODO: 方法区是规范，元空间和永久代是方法区的具体实现
    //    元空间和永久代是不是同时存在的 ?
    //      < jdk1.8  方法区的具体实现叫永久代，放在堆上，触发GC，产生OOM
    //      >= jdk1.8 方法区的具体实现叫元空间，放在本地内存(OS内存，直接内存，native memory)
    //                操作系统回收，减少GC压力
    //    为什么要用元空间代替永久代 ?
    //      永久代，放在堆上，容易产生OOM
    ...
    //    4.2 TODO: 使用"类型名_本地方法名称"在底层openjdk源码中可以找到对应的实现
    //    4.3 本地方法栈一直存在吗? 何时存在?
    ...
    // 3. JVM底层使用C++来实现，C++生成的对象默认是在native method，是如何放到JVM堆上的?
}
