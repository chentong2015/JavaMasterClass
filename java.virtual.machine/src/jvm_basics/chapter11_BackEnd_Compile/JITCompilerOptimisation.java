package jvm_basics.chapter11_BackEnd_Compile;
...
public class JITCompilerOptimisation {
    ...
    // 1. 方法内联(编译原理)
    //    将目标方法的的代码"Copy"到调用该方法的方法中，避免发生真实的方法调用而已
    ...
    //    2.1 TODO: 将对象之间在线程栈上分配内存空间，然后随着方法的结束而销毁，减少堆上的GC压力
    //    2.2 标量替换，将Java对象打散，创建若干个被这个方法使用的成员变量来替代
    //    2.3 没有线程逃逸，则没有同步竞争，则可以消除对这个变量的同步措施
    public int testEscapeAnalysis(int x) {
        // int xx = x + 2;
        // Point p = new Point(xx, 42); 由于没有逃逸，则可以直接将x,y坐标置换出来，分解成局部变量，避免创建Point对象
        // return p.getX();
