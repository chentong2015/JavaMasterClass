...
public class BaseGarbageCollection {
    ...
    // 5. 客户端调用/主动通知JVM: System.gc()或Runtime.getRuntime().gc(), 但JVM可以屏蔽掉显式垃圾回收调用
    ...
    // 3. 并发可达性分析算法:
    //    在并发标记的过程中，由于标记期间应用程序还在继续跑，对象间的引用可能发生变化，造成
    ...
    //    > 白色: 表示对象没有被GC收集器访问过，如果在分析结束阶段仍然是白色的，说明该对象不可达
    // 三色标记算法两大问题
    //    > 多标: 浮动垃圾
    //    > 漏标: 读写屏障 -> 增量更新(黑色插入对白色对象的引用，则变成灰色)/原始快照
