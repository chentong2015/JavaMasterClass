...
/**
 * PreparedStatement 提供通配符?形式 ==> 能够解决SQL Injection注入问题
 * PreparedStatement将?符号所替换的信息处理成字面值，而非解释成带sql语句的字符串(当字符串中含有sql关键字时) !!
 */
public class PreparedStatementDemo {
    ...
    /**
     * 针对需要根据条件筛选的查询语句
     * SQL Injection通过在SQL筛选条件中追加一个判断or判断条件，使得查询语句能够检索出table中的所有信息
     * 使得WHERE判断条件失效，返回了非相关用户的信息，造成信息泄露
     */
    private void testSQLInjection(String searchName) {
        String queryNotOK = "SELECT * FROM users_view WHERE name=\"chen\" or 1=1 or \"\""; // 注入or关键字

        String queryOK = "SELECT * FROM users_view WHERE name=?"; // ? placeholder character
    ...
    /**
     * preparedStatement.executeUpdate() 指定执行数据操作语句, 或者没有数据返回的查询statement !!
