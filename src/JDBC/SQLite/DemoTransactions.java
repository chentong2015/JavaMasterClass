...
/**
 * SQLite 默认使用transactions和auto commit来执行对数据库的更新 !!
 * JDBC Connection默认在SQL Statement结束瞬间会立即更新数据库，但是在某些场景下，不能在每个statement结束自动commit
 * ---------------------------------------------
 * Transactions：A sequence of SQL statement that are treated as a single logical unit
 ...
 * 只需要在"change data in database"改变数据的时，才有必要使用transactions
 * 1. Atomicity: a series of sql statements; either all changes are committed, or none of them are 原子性
 * 2. Consistency: before and after the transactions, the database is in a valid state 前后的状态一致性
 ...
 * 4. Rollback: roll back any uncommitted changed and ends the transactions 只能回滚到上一次commit或者Rollback的状态 !!
 */
public class DemoTransactions {
    ...
    public void insertSongByTitle(String title, String artistName, String album, int track) {
        ...
        try (PreparedStatement insertIntoSongs = connection.prepareStatement(insertSongs);) {
            ...
            if (affectedRows == 1) {
                connection.commit(); // 所有的transactions序列sql statement操作都成功，则commit changes to database !!
            } else {
            ...
            try {
                System.out.println("Performing rollback"); // 回滚，执行数据的恢复操作
                connection.rollback();
