...
//    需要使用一个private long filed => serialVersionUID;
//    如果不是自定义，将会被赋予一个默认值:
//        编译器会根据类型包含的fields和其类型details生成，在类型变动之后，当反序列化时runtime验证serialVersionUID可能会出现不一致
//        在不同的compiler之间可能存在差异: Standard Oracle Compiler for Desktop APP & Android Project 不同编译器序列化效果不同
// 3. 类型中的所有的field都必须是可以序列化的
// 4. 将同一个对象序列化两次到一个文件中，文件中之后存储一次对象的序列化结果
