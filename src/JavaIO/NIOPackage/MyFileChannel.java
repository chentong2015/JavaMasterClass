...
public class MyFileChannel {
    ...
    private static void testReadDataFromBinaryFile() throws IOException {
        ...
        long numBytesRead = channel.read(buffer); // 读取到buffer所包装的字节数组，注意buffer的position的位置 !!!!
        if (buffer.hasArray()) {
        ...
        intBuffer.flip();
        int readInt = intBuffer.getInt(); // getInt(0) "absolute read"不使用.flip() ==> 不会设置index position=0的位置
