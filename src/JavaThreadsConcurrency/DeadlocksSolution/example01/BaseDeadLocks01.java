...
/**
 * DeadLock解决方案：wait(), notify(), notifyAll()
 * 1. 在线程被阻塞的时候，等待，在线程结束操作时候，唤起等待
 * 2. 只能在非静态方法中调用吗, 锁所关联的都是object's monitor
 * 3. 只能在synchronized block语句块中调用
 * 4. Thread线程必须要拥有object对象上的lock, 然后才能调用wait()
 * .
 ...
 * 4. TODO: 当多个线程同时想要获得锁时，最后所获得锁的线程是由JVM所决定的，并不是最先来的Thread ==> 线程饥饿问题 !!
 */
public class BaseDeadLocks01 {
    ...
    public void testInterrupt() {
        DeadLockMessage message = new DeadLockMessage();
        Thread writeThread = new Thread(new ThreadWriter(message));
        writeThread.start();
        Thread readThread = new Thread(new ThreadReader(message));
        readThread.start();
