...
/**
 * Synchronisation: the process of controlling when threads execute codes and access the heap space 同步化: 顺序执行 !!
 * 1. 可以同步方法, 实例方法或者静态方法(除了构造器方法之外)
 * 2. 可以同步Block of Statements, 一段语句块
 * 3. Critical Section: Shared resource like a variable, only one thread can execute critical section
 * 4. Thread-Safe: Has synchronized all the critical sections within the code 线程安全的，如果不是，则需要自定义添加
 * 5. 需要同步所有可能出现多线程interferences的地方 !!!
 * 6. 最大限度的同步最小的语句块，避免因为同步对线程造成中断，影响程序的性能 !!!
 */
public class Synchronisation {
     ...
     * 4. TODO: Threads will create their own copy of object 每个线程中对象的引用不一致  ===> 深入理解Java虚拟机
