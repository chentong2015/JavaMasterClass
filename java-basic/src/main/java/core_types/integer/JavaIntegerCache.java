...
public class JavaIntegerCache {
    ...
    public static void main(String[] args) {
        ...
        Integer aaa = 200;
        Integer bbb = 200;                   // new Integer(200) 缓存值范围外，直接构建堆上新的对象
        System.out.println(aaa == bbb);      // false  比较引用，两个对象的引用不同
        ...
        Integer c = 3;
        Long g = 3L;
        System.out.println(c == a + b);      // true   运算后自动拆箱，int和int比较值
        System.out.println(c.equals(a + b)); // true   运算后自动装箱, 比较Integer类型对象的值

        System.out.println(g == a + b);      // ture   运算后自动拆箱, int和long值比较时自动隐式转换
        System.out.println(g.equals(a + b)); // false  运算后自动装箱, 但两个引用类型不同
    }
