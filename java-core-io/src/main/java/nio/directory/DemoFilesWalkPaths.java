...
public class DemoFilesWalkPaths {
    ...
    private void testWalkFileTree() {
        try {
            Path rootFolder = FileSystems.getDefault().getPath("java-core-io/folder");
    ...
    private void testCopyFileTree() {
        Path sourceDir = FileSystems.getDefault().getPath("java-core-io/folder");
        Path targetDir = FileSystems.getDefault().getPath("java-core-io/folderCopy"); // 结果的总文件夹不能已经存在 !!
        try {
