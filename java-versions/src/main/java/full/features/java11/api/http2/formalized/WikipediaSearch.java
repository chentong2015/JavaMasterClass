...
public class WikipediaSearch {
    ...
    public static void main(String[] args) {
        List<PageSearch.Search> searches = URLS.stream()
                .map(url -> URI.create(url))
                .map(url -> new PageSearch.Search(url, SEARCH_STRING))
                .collect(toList());
    ...
    private static void search(PageSearch pageSearch, List<PageSearch.Search> searches) {
