...
public class ConcurrentHashMap7 {
    ...
    //       这里的segmentShift和segmentMask有对应关系，保证"与计算"的bit位置上能够取到  ===> 两次算下标其实是相互独立的, 为啥多个"与计算"
    //       int segmentIndex = (hash >>> segmentShift) & segmentMask;
    //       UNSAFE.getObject(segments, (j << SSHIFT) + SBASE); TODO: 通过UNSAFE方式取数组中指定位置的值
    //       判断对应的位置是否有Segment的对象
    //       如果没有则会创建s.ensureSegment(segmentIndex);
