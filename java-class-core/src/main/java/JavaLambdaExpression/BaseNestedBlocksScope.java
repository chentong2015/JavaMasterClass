...
import JavaLambdaExpression.base.IStringUpperConcat;

import java.util.ArrayList;
import java.util.List;

// Lambda expressions的作用域和变量捕获
// 1. "匿名方法能够使用声明该匿名方法的方法内部定义的局部变量"
//    ==> 可以避免创建额外的类来存储信息
// 2. 外部变量: 在它的作用域中声明了一个匿名方法，并且匿名方法可以对它进行捕获
//    ==> 外部变量的更改在匿名方法内部是可见的
// 3. "当一个变量被捕获时，捕获的是变量的<实例>"
//    ==> 如果是在循环内部，则会创建多个实例化对象(被捕获), 彼此之前不共享
...
public class BaseNestedBlocksScope {
    ...
    // 在Lambda expressions中所使用的作用域中的局部变量，必须声明成final或者确定不变
    ...
    // 在循环中，testNumbers list中的对象在变化，但是list中所存储的对象的引用没有变化
    // testNumbers是effectively final
    ...
    /**
     * 特别注意：在使用循环时，lambda表达式中所使用的变量是否是不变的 !!
    ...
    private void testForScope() {
        ...
        // 这里可以不需要申明成final，item变量的值是effectively final的
